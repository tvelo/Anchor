  ...
  return (
    <View style={{ width: SW, height: CONTENT_H, backgroundColor: '#000', alignItems: 'center', justifyContent: 'center' }}>
      <View style={{ width: srcW, height: srcH, backgroundColor: page.bgColor || '#FFF8F0', overflow: 'hidden', transform: [{ scale }] }}>
        ...
        {photos.map((el: any) => (
          <FilteredImage key={el.id} uri={el.url} filter={el.filter} style={{
  ...
  const handlePost = async () => {
    ...
    if (data) {
      ...
      if (post.user_id !== currentUserId) {
        notifyMembers({ type: 'social_comment', target_user_id: post.user_id, actor_id: currentUserId, actor_name: profile?.display_name ?? 'Someone', title: 'New comment 💬', body: `${profile?.display_name ?? 'Someone'} commented: "${text.slice(0, 50)}"` }).catch(() => {});
      }
    }
    setPosting(false);
  ...
  return (
    <View style={{ width: SW, height: SH, backgroundColor: '#111' }}>
      ...
      <View style={{ width: SW, height: CONTENT_H, overflow: 'hidden', backgroundColor: '#000' }}>
        {!isVisible || loading || slides.length === 0 ? (
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            ...
            {loading && isVisible && <ActivityIndicator color={C.accent} style={{ position: 'absolute' }} />}
          </View>
        ) : (
          <FlatList
            ref={innerRef}
...
function ProfileView({ profile, currentUserId, isOwnProfile, onBack, onFollowChange, onPostPress }: {
  ...
  const handleFollow = async () => {
    ...
    if (isFollowing) {
      ...
      const { data: myProf } = await supabase.from('social_profiles').select('display_name').eq('id', currentUserId).maybeSingle();
      notifyMembers({ type: 'social_follow', target_user_id: profile.id, actor_id: currentUserId, actor_name: myProf?.display_name ?? 'Someone', title: 'New follower 👤', body: `${myProf?.display_name ?? 'Someone'} started following you` }).catch(() => {});
    }
    onFollowChange?.();
  };
        ...
        <View style={st.profileHero}>
          ...
          {!isOwnProfile && (
            <View style={{ flexDirection: 'row', gap: 10, justifyContent: 'center' }}>
              ...
              <TouchableOpacity
                style={[st.followBtn, { backgroundColor: C.surface, borderWidth: 1, borderColor: C.border }]}
                onPress={async () => {
                  try {
                    const { getOrCreateConversation } = await import('../../lib/api/messaging');
                    const convoId = await getOrCreateConversation(currentUserId, profile.id);
                    const { router: r } = await import('expo-router');
                    r.push(`/messages/${convoId}`);
                  } catch {}
                }}>
                <Text style={[st.followBtnText, { color: C.textPrimary }]}>✉️ Message</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
...
export default function SocialScreen() {
  ...
  const handleLike = async (postId: string, liked: boolean) => {
    ...
    if (liked) {
      await supabase.from('social_likes').upsert({ user_id: userId, post_id: postId }, { onConflict: 'user_id,post_id' });
      // Notify post owner
      const post = fypPosts.find(p => p.id === postId) ?? followingPosts.find(p => p.id === postId) ?? discoverPosts.find(p => p.id === postId);
      if (post && post.user_id !== userId) {
        notifyMembers({ type: 'social_like', target_user_id: post.user_id, actor_id: userId, actor_name: myProfile?.display_name ?? 'Someone', title: 'New like ❤️', body: `${myProfile?.display_name ?? 'Someone'} liked your post` }).catch(() => {});
      }
    } else {
      await supabase.from('social_likes').delete().eq('user_id', userId).eq('post_id', postId);
    }
  };
